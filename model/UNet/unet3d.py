...
class ResUNet3D(nn.Module):
    def __init__(self, n_channels, n_classes, f_maps=8,
                 levels=4, residual_block=True, se_block="CSSE", attention=True,
                 trilinear=True, MHTSA_heads=4, MHGSA_heads=0, MSSC="None"):
        ...
        self.MSSC = MSSC
        feature_maps = (f_maps * np.exp2(np.arange(levels+1))).astype(int)
        ...
        self.encoder = nn.ModuleList([Down(feature_maps[i],
                                           feature_maps[i+1],
                                           residual_block=residual_block,
                                           se_block=se_block,
                                           pooling_size=pooling_sizes[i]) for i in range(levels-1)])


        self.bridge = nn.Sequential(Down(feature_maps[-2],
                                         feature_maps[-1],
                                         residual_block=residual_block,
                                         se_block=se_block),
        ...
        self.decoder = nn.ModuleList([Up(feature_maps[i],
                                         feature_maps[i-1],
                                         trilinear,
                                         residual_block=residual_block,
                                         se_block=se_block,
                                         ...
                                         MHCA_heads=0,
                                         multi_scale_sc=MSSC,
                                         up_channels=up_channels[i],
                                         up_scale_factor=up_scale_factors[i]) for i in range(1, levels+1)[::-1]])
